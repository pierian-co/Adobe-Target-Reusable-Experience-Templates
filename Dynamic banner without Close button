...
let c=document.querySelector(".offerbox_out"),g=document.querySelector(".fadecontent");document.getElementById("ignoreRET");l.onclick=function(){f.classList.toggle("arrowChecked"),d.classList.toggle("openedBox"),c.classList.contains("slide-from-top")?(c.classList.remove("slide-from-top"),c.classList.add("slide-from-bottom")):(c.classList.remove("slide-from-bottom"),c.classList.add("slide-from-top"))},"yes"!=config.logoImage.flag&&(document.querySelector("#logoImage").style.display="none"),"yes"==config.autoHide.flag&&setTimeout(function(){document.querySelector(".mid").classList.add("slide-from-bottom")},1e3*config.autoHide.time),clearInterval(delay)}},config.initialDelay.time);
