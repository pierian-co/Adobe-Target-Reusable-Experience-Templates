...
if(config.background.imageUrl!=""){
	document.querySelector(".mid").style.backgroundImage = "url('" + config.background.imageUrl + "')";
}
if(config.background.imageUrl==""){
	document.querySelector(".mid").style.backgroundColor= config.background.backgroundColour;
}

// Define where to insert the Header of the Modul
 // insert the Header of the Modul
/*-----------------------------------------*/
/*  INSERT CAMPAIGN SPECIFIC CONTENT HERE  */
/*-----------------------------------------*/
let s_headline = config.message.text;
let s_benefit1 = config.subHeading.subHeading1Text;
let s_benefit2 = config.subHeading.subHeading2Text;
let s_benefit3 = config.subHeading.subHeading3Text;
let s_ctared = config.ctaLink1.text;
let s_ctawhite = config.ctaLink2.text;
let s_ctared_link = config.ctaLink1.href;
let s_ctawhite_link = config.ctaLink2.href;
/*-----------------------------------------*/
/*-----------------------------------------*/
let offerBadgeOut = document.querySelector('.offerbadge_out'); // Define where to insert Box of Modul
let offerBadgeInner = document.querySelector('.offerbadge');
let open_close = document.querySelector('.inflex2');
offerBadgeOut.insertAdjacentHTML('afterend', '<div class="offerbox_out"> <div><div class="offerbox_headline"><span class="s_headline" style="color:'+config.message.colour+'">'+s_headline+'</span> </div><div class="offerbox_in"><div class="innerflex1"> <div class="benefits"> <div class="inner_benefits"> <i class="check_per"></i> <a href=""><span class="innerspan">'+s_benefit1+'</span></a> </div><div class="inner_benefits"><i class="check_per"></i> <a href=""><span class="innerspan">'+s_benefit2+'</span></a> </div><div class="inner_benefits"><i class="check_per"></i> <a href=""><span class="innerspan">'+s_benefit3+'</span></a> </div></div></div><div class="innerflex2"> <a class="red-cta-link" href="'+s_ctared_link+'"><button type="button" class="red-cta">'+s_ctared+'</button></a> <a class="white-cta-link" href="'+config.ctaLink2.href+'"><button type="button" class="white-cta">'+s_ctawhite+'</button></a> </div></div></div></div>');
if(config.ctaCount.singleCTA !="none"){
	document.querySelector(".white-cta-link").style.display="none";
	
}
if(config.ctaLink2.iscloseButton=="yes"){
	document.querySelector(".white-cta-link").setAttribute("href","javascript:void(0)");
	document.querySelector(".white-cta-link").onclick=function(){document.querySelector(".mid").classList.add("slide-from-bottom")}
}
...
if (config.autoHide.flag == "yes") {
                        setTimeout(function() {
                            document.querySelector(".mid").style.display="none";
                            
                        }, config.autoHide.time * 1000);
                    }
