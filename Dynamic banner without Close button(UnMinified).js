...
var delay = setInterval(function() {
    if (0 == document.querySelectorAll(".mid").length) {
        ...
        }, "yes" != config.logoImage.flag && (document.querySelector("#logoImage").style.display = "none"), "yes" == config.autoHide.flag && setTimeout(function() {
            document.querySelector(".mid").classList.add("slide-from-bottom")
        }, 1e3 * config.autoHide.time)}else{console.log("selector where we need to insert banner is not present. Please check the selector")} 
        clearInterval(delay)
    }
